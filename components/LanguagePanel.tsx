  ...
  return (
    <div className="flex flex-col h-full">
        ...
        <div className="flex items-center gap-2">
            {onRecordClick && (
                <button 
                    ...
                    className={`p-2 rounded-full transition-colors duration-200 ${isRecording ? 'bg-red-500/20 text-red-500' : 'text-slate-500 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700'} disabled:opacity-50 disabled:cursor-not-allowed`}
            ...
            {onSpeakClick && (
                <button 
                    ...
                    disabled={!value.trim()}
                    className="p-2 rounded-full transition-colors duration-200 text-slate-500 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700 disabled:text-slate-400 dark:disabled:text-slate-600 disabled:cursor-not-allowed"
      ...
      <textarea
        ...
        readOnly={readOnly}
        rows={5}
        className={`w-full p-4 text-base border-2 rounded-xl transition-colors duration-200 focus:outline-none focus:ring-2 resize-none
          ${
            readOnly
